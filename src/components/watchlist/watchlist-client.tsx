...
import {
  ...
  STOCK_ROW_HEIGHT,
} from "@/components/watchlist/stock-row";
...
const AddStockModal = dynamic(
  () =>
    import("@/components/watchlist/add-stock-modal").then((m) => ({
      default: m.AddStockModal,
    })),
  { ssr: false },
);
const NotesModal = dynamic(
  () =>
    import("@/components/watchlist/notes-modal").then((m) => ({
      default: m.NotesModal,
    })),
  { ssr: false },
);
...
export function WatchlistClient({ watchlistId }: WatchlistClientProps) {
  ...
  const [sortField, setSortField] = useState<
    "symbol" | "pChange" | "marketCap" | null
  >("marketCap");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const sortFieldRef = useRef<"symbol" | "pChange" | "marketCap" | null>(
    "marketCap",
  );
    ...
    if (sortField) {
      list.sort((a, b) => {
        ...
        if (sortField === "symbol") {
          ...
          av =
            (priceMap as Record<string, StockPrice>)[a.symbol]?.pChange ??
            -Infinity;
          bv =
